...
const showCategory = async (req, res) => {
    try {
        ...
        res.render('admin_category', { listData, categoryFound, username: req.session.username })
...
const addCategory = async (req, res) => {
    try {
        ...
        const categoryFound = await catMod.find({ name: { $regex: new RegExp(req.body.category, 'i') } })
...
const categoryEdit = async (req, res) => {
    try {
        ...
        if (categoryFound.name == req.body.name) {
            ...
            await catMod.updateOne({ name: req.params.id }, { $set: { name: req.body.name } })
